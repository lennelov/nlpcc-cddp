...
def get_result(correct_chunks, true_chunks, pred_chunks,
    ...
    logger.info("processed %i tokens with %i phrases; " % (sum_true_counts, sum_true_chunks))
    logger.info("found: %i phrases; correct: %i.\n" % (sum_pred_chunks, sum_correct_chunks))
        
    logger.info("accuracy: %6.2f%%; (non-O)" % (100*nonO_correct_counts/nonO_true_counts))
    logger.info("accuracy: %6.2f%%; " % (100*sum_correct_counts/sum_true_counts))
    logger.info("precision: %6.2f%%; recall: %6.2f%%; FB1: %6.2f" % (prec, rec, f1))
...
def entity_f1(true_seqs, pred_seqs, verbose=True):
...
def evaluate_conll_file(fileIterator):
    ...
    return entity_f1(true_seqs, pred_seqs)
